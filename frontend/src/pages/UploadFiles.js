...
const findMultiLevelMfgPartsHeaderRow = (rows = []) => {
  const normalizedRows = rows.map(normalizeRowCells);
  for (let rowIndex = 0; rowIndex < Math.min(normalizedRows.length, HEADER_SCAN_ROWS); rowIndex += 1) {
    const row = normalizedRows[rowIndex];
    const keys = row.map(normalizeHeaderKey);
    const descriptionIndex = keys.findIndex(key => key === 'description');
    const hasQuantity = keys.some(key => key === 'qty' || key === 'quantity' || key === 'qnty');
    const hasMfgParts = keys.some(key => key === 'mfg parts' || key === 'mfg part' || key === 'manufacturer parts' || key === 'manufacturer part');
    if (descriptionIndex <= 0 || !hasQuantity || !hasMfgParts) continue;

    const leadingColumns = row.slice(0, descriptionIndex);
    const leadingHeadersAreBlank = leadingColumns.every(cell => !cell);
    if (!leadingHeadersAreBlank) continue;

    const belowRows = normalizedRows.slice(rowIndex + 1, rowIndex + 25);
    const itemRows = belowRows.filter(nextRow => (
      nextRow.slice(0, descriptionIndex).some(cell => /[A-Za-z0-9]/.test(cell)) &&
      nextRow[descriptionIndex]
    ));
    const mfgBlockRows = belowRows.filter(nextRow => (
      nextRow.some(cell => /manufacturer part number|part numbers|manufacturers/i.test(cell))
    ));
    if (itemRows.length >= 3 && mfgBlockRows.length >= 1) return rowIndex + 1;
  }
  return 0;
};
...
const scoreHeaderRow = (rows = [], rowIndex = 0) => {
  ...
  return (
    ...
    + keywordHits * 8                  // known BOM/table labels are strong signals
    + (keywordHits >= 3 ? 18 : 0)
    ...
    - longNumericIdentifiers * 18      // item-code rows are data, not headers
