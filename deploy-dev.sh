...
# Excel Template Mapper - DEV Environment Fresh Deployment Script
# This script completely rebuilds and deploys your application to DEV with fresh code
...
REGISTRY_NAME="excelmapperregistry96552"
REGISTRY_URL="${REGISTRY_NAME}.azurecr.io"
IMAGE_NAME="excel-template-mapper"
WEBAPP_NAME="excel-mapper-backend-96552"
RESOURCE_GROUP="excel-mapper-rg-96552"

echo -e "${PURPLE}🚀 Excel Template Mapper - DEV Environment Fresh Deployment${NC}"
echo -e "${PURPLE}=========================================================${NC}"
echo -e "${BLUE}📍 Target: ${WEBAPP_NAME}.azurewebsites.net${NC}"
echo ""

# Step 1: Get tag name from user
echo -e "${BLUE}📝 Step 1: Tag Selection${NC}"
echo -e "${YELLOW}Please enter a tag name for your DEV deployment:${NC}"
echo -e "${YELLOW}Examples: dev-v1, multiple-canonical-mpns, package-brackets-fix${NC}"
...
echo -e "${BLUE}🔄 Step 8: Updating Azure Web App (DEV)${NC}"
...
echo -e "${BLUE}⏳ Step 9: Verifying DEV deployment${NC}"
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    ...
    if [ "$HTTP_CODE" = "200" ]; then
        echo -e "${GREEN}✅ DEV application is running successfully!${NC}"
        break
    else
...
echo -e "${BLUE}📋 Step 10: DEV Deployment Summary${NC}"
...
echo ""
echo -e "${BLUE}🌐 DEV Application URLs:${NC}"
...
echo -e "  Data Editor: https://${WEBAPP_NAME}.azurewebsites.net/editor/"
echo ""
...
echo -e "${PURPLE}✨ Your DEV deployment with tag '${TAG_NAME}' is ready for testing!${NC}"
echo -e "${PURPLE}   Test your multiple canonical MPNs with package brackets feature now.${NC}"
echo ""

# Optional: Show recent tags for reference
echo -e "${BLUE}📋 Recent tags in DEV registry:${NC}"
az acr repository show-tags --name ${REGISTRY_NAME} --repository ${IMAGE_NAME} --output table --orderby time_desc --top 5 2>/dev/null || echo -e "${YELLOW}Could not retrieve recent tags${NC}"
echo ""

# Optional: Open the application in browser (uncomment if desired)
# echo -e "${YELLOW}Opening DEV application in browser...${NC}"
